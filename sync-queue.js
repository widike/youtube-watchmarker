...
export class SyncQueue {
  ...
  async init(supabaseProvider) {
    ...
    this.startAutoSync();
    logger.info(
      `Sync queue initialized with ${this.queue.length} pending operations`,
    );
  }
  ...
  async loadPersistedQueue() {
    try {
      ...
      if (result[this.storageKey] && Array.isArray(result[this.storageKey])) {
        ...
        logger.debug(
          `Loaded ${this.queue.length} operations from persisted queue`,
        );
      }
    } catch (error) {
  ...
  async enqueue(type, data) {
    ...
    logger.debug(
      `Enqueued ${type} operation, queue size: ${this.queue.length}`,
    );
  ...
  async trySync() {
    ...
    if (!circuitStatus.isAvailable) {
      logger.debug(
        `Circuit breaker open, skipping sync (state: ${circuitStatus.state})`,
      );
      return;
    }
    ...
    if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
      ...
      const backoffTime = Math.min(
        this.consecutiveFailures * this.retryDelayMs,
        300000,
      ); // Max 5 min
      if (timeSinceLastAttempt < backoffTime) {
        logger.debug(
          `Backing off sync, ${Math.ceil((backoffTime - timeSinceLastAttempt) / 1000)}s remaining`,
        );
        return;
      }
    }
  ...
  async processQueue() {
    ...
    const putOps = this.queue.filter(
      (op) => op.type === SyncOperationType.PUT_VIDEO,
    );
    const otherOps = this.queue.filter(
      (op) => op.type !== SyncOperationType.PUT_VIDEO,
    );
