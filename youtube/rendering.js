...
(() => {
  ...
  class PublicationDateManager {
    constructor(settingsStore) {
      this.settingsStore = settingsStore;
      this.publishDates = new Map();
    }

    async processVideos(videos) {
      const isEnabled = await this.settingsStore.get(
      ...
      for (const video of videos) {
        this.applyPublishDate(video, core.extractVideoId(video));
      }
    }

    applyPublishDate(videoElement, videoId) {
      if (!videoId) {
        return;
      }

      const publishDate =
        this.publishDates.get(videoId) || core.extractPublishDate(videoElement);
      if (!publishDate) {
        return;
      }

      this.publishDates.set(videoId, publishDate);

      const titleElement = core.findVideoTitleElement(videoElement);
      if (
        !titleElement ||
        titleElement.dataset.publishDateApplied === videoId
      ) {
        return;
      }

      if (titleElement.dataset.publishDateBaseText === undefined) {
        titleElement.dataset.publishDateBaseText =
          titleElement.textContent?.trim() || "";
      }

      const baseText = titleElement.dataset.publishDateBaseText || "";
      titleElement.textContent = `${baseText} • ${publishDate}`;
      titleElement.dataset.publishDateApplied = videoId;
    }
    ...
    clear() {
      ...
      document
        .querySelectorAll("[data-publish-date-base-text]")
        .forEach((element) => {
          element.textContent = element.dataset.publishDateBaseText || "";
          delete element.dataset.publishDateBaseText;
          delete element.dataset.publishDateApplied;
        });
    }
  }
  ...
  watchmarker.PublicationDateManager = PublicationDateManager;
})();
